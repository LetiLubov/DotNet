...
namespace Nosql
{
    class Program
    {
        ...
        static void InsertInMSSQL(MyProduct p) //записываем в MSSQL
        {
            ...
            aMsSql = context.Author.FirstOrDefault(x => x.author1 == p.Author);
            if (aMsSql == null) //нет в базе, узнаем новый id для него, добавляем производителя
            {
                aMsSql = new Author();
                aMsSql.author1 = p.Author;
                Author aMsSqlH = context.Author.OrderByDescending(u => u.id).FirstOrDefault();
                if (aMsSqlH != null)
                    aMsSql.id = aMsSqlH.id + 1;
                else
                    aMsSql.id = 0;
        ...
        static void PrintListInfoAboutSimilarProducts(Author aMsSql2)
        {
